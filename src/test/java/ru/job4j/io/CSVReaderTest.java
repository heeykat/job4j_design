...
class CSVReaderTest {
    ...
    @Test
    void whenFilterOneColumns(@TempDir Path folder) throws Exception {
        ...
        ArgsName argsName = ArgsName.of(new String[]{
                ...
                "-out=" + target.getAbsolutePath(), "-filter=last_name"});
    ...
    @Test
    void whenEmptyCSV(@TempDir Path folder) throws Exception {
        String data = "";
        File file = folder.resolve("source.csv").toFile();
        File target = folder.resolve("target.csv").toFile();
        ArgsName argsName = ArgsName.of(new String[]{
        ...
        assertThatThrownBy(() -> CSVReader.handle(argsName))
                ...
                .hasMessageContaining("The CSV file is empty or unreadable");
    }
}